    ...
    def execute(self, tool_name, tool_info, arguments):
        ...
        try:
            if tool_name == "exec_python_file":
                ...
                # Web search is handled by OpenAI API directly, results are in the response
                # This handler is here for completeness but shouldn't normally be called
                tool_info["content"] = (
                    "Web search was performed by the model. Results should be in the assistant's response."
                )
                return tool_info
            else:
    ...
    def run(self):
        ...
        while self.message_queue:
            ...
            while round < config.MAX_ROUNDS:
                ...
                # Check for both old function_call format and new tool_calls format
                has_function_call = "function_call" in assistant_output
                has_tool_calls = (
                    "tool_calls" in assistant_output and assistant_output["tool_calls"]
                )
            ...
            while round < config.MAX_ROUNDS:
                ...
                if "function_call" in assistant_output:
                    # Legacy format
                    ...
                    # New GPT-5 format - handle multiple tool calls
                    for tool_call_item in assistant_output["tool_calls"]:
                        tool_call = tool_call_item.get("function", {})
                        tool_name = tool_call.get("name", "")
                        tool_call_id = tool_call_item.get("id", "")

                        # Handle web_search tool (handled by OpenAI, results already in response)
                        if (
                            tool_call_item.get("type") == "web_search"
                            or tool_name == "web_search"
                        ):
                            # Web search results are handled by OpenAI API directly
                            # The results should already be in the assistant_output content
                            self.logger.info("Web search was performed by the model")
                            # Continue to next iteration to get the response with search results
                            continue
                        ...
                        if tool_info == {}:
                            break
                        # Format tool response for GPT-5 API
                        tool_info["tool_call_id"] = tool_call_id
                        self.add_memory(tool_info)
                        req += [tool_info]
                else:
                    break
                ...
                # Check for next tool calls
                has_function_call = "function_call" in assistant_output
                has_tool_calls = (
                    "tool_calls" in assistant_output and assistant_output["tool_calls"]
                )
                while not has_function_call and not has_tool_calls:
                    req += [
                        {
                            "role": "user",
                    ...
                    has_function_call = "function_call" in assistant_output
                    has_tool_calls = (
                        "tool_calls" in assistant_output
                        and assistant_output["tool_calls"]
                    )
                    round += 1
