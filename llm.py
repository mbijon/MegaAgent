...
def gen_tools(agent_name):
    ...
    # Add web search tool if enabled (for GPT-5 models)
    if config.enable_web_search:
        tools.append(
            {
                "type": "web_search",
                "name": "web_search",
                "description": "Search the web for current information. Use this tool when you need up-to-date information, facts, or data that may not be in your training data.",
            }
        )
...
def _get_llm_response(messages, enable_tools=True, agent_name=""):
    ...
    if enable_tools:
        # For GPT-5 models, use "tools" format; for older models, use "functions"
        # Check if model is GPT-5 or newer
        is_gpt5_or_newer = (
            config.model.startswith("gpt-5") or "gpt-5" in config.model.lower()
        )

        if is_gpt5_or_newer:
            # Convert function-style tools to tool format for GPT-5
            tools_formatted = []
            for tool in tools:
                if tool.get("type") == "web_search":
                    # Web search tool (built-in for GPT-5)
                    tools_formatted.append({"type": "web_search"})
                else:
                    # Function-style tool
