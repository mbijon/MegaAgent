...
- `MEGAAGENT_MODEL`, `OPENAI_CHAT_URL`, and `MEGAAGENT_ENABLE_WEB_SEARCH` let you change the deployed model, API endpoint, or diable the GPT-5 web search tool.
- `MEGAAGENT_WEB_SEARCH_MODEL` / `MEGAAGENT_WEB_SEARCH_PROVIDER` fine-tune which search backend is attached to chat completions. Note we are using the Chat API: <https://platform.openai.com/docs/guides/tools-web-search?api-mode=chat&lang=python>
