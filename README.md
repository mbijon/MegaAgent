...
MegaAgent now uses **GPT-5.1** by default with support for web search capabilities. Configuration is managed through environment variables for security:

- `OPENAI_API_KEY`: Your OpenAI API key (required)
- `OPENAI_MODEL`: Model to use (defaults to `gpt-5.1`)
- `ENABLE_WEB_SEARCH`: Enable web search tool (defaults to `true`)

To run the latest version, set your API key as an environment variable and optionally customize the model:

```bash
export OPENAI_API_KEY='your-api-key-here'
export OPENAI_MODEL='gpt-5.1'  # Optional, defaults to gpt-5.1
export ENABLE_WEB_SEARCH='true'  # Optional, defaults to true
python main.py
```

The output will be located at `files/`. The default prompt is used for automatically generating a Gobang game.
...
Tests are located in the `tests/` directory and include comprehensive coverage of core functionality, including web search capabilities. All external HTTP calls are mocked to avoid hitting the OpenAI API during testing.
